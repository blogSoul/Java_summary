1. 오름차순, 내림차순을 만들 때, 사용할 규칙에 맞게 각각 설정해줍니다.
2. 분할정복이므로 간단하게 계산할 수 있을만큼 나눠줄 수 있도록 합니다.
3. 같은 점에 대한 처리를 할 수 있도록 합니다.
4. sort를 하는 이유는 좀 더 계산량을 줄일 수 있는 상황에서 사용할 수 있도록 합니다. (sort자체가 비싼 계산량입니다.)
...
public class Main {
    ...
    public static int Checkshortest(List<Pointer> list, int x, int y){
        int mid = (x + y) / 2;
        int left = Findshortest(list, x, mid);
        int right = Findshortest(list, mid + 1, y);
        int num = Math.min(left, right);
        List<Pointer> list_short = new ArrayList<Pointer>();
        for(int i = x; i <= y; i++){
            int distanceX = list.get(i).getX() - list.get(mid).getX();
